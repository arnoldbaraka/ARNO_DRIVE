...
// };
// Replace TensorFlow with lightweight alternative
export const analyzeSentiment = async (text) => {
  // Simple regex-based sentiment analysis
  const positive = /(awesome|great|good|love|like|excellent)/gi;
  const negative = /(bad|terrible|awful|hate|dislike)/gi;
  
  const posMatches = text.match(positive) || [];
  const negMatches = text.match(negative) || [];
  
  if (posMatches.length > negMatches.length) return 'positive';
  if (negMatches.length > posMatches.length) return 'negative';
  return 'neutral';
};
